...
for name in ['PS', 'OEFR', 'SIR', 'PP', 'TCT', 'CIV', 'CKR', 'CAFR']:
    if name in INDUSTRIAL_RANGES:
        ...
        parameters.append({'name': name, 'bounds': bounds})
        logger.info(f"使用数据范围 {name}: {bounds}")
    else:
...
# --- 8. 绘制敏感性指数图 ---
# Convert the ResultDict to a DataFrame
...
# # 绘制一阶指数
# ax1 = bar_plot(Si_df, index='S1', ax=axes[0])
# ax1.set_title('一阶敏感性指数 (S1)', fontsize=14)
# ax1.set_ylabel('敏感性指数', fontsize=12)

# # 绘制总指数
# ax2 = bar_plot(Si_df, index='ST', ax=axes[1])
...
# 一阶敏感性指数
axes[0].bar(names, S1)
axes[0].set_title('一阶敏感性指数 (S1)')
axes[0].set_ylabel('敏感性指数')

# 总敏感性指数
axes[1].bar(names, ST)
axes[1].set_title('总敏感性指数 (ST)')
axes[1].set_ylabel('敏感性指数')

plt.tight_layout()
plt.savefig('sobol_sensitivity_manual.png', dpi=300)
logger.info("手动绘制的敏感性指数图已保存为 sobol_sensitivity_manual.png")
